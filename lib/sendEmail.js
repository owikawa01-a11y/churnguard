...
export async function sendCancellationAlert({
  ...
  if (!resend) {
    console.warn('[ChurnGuard][email] RESEND_API_KEY missing, skipping');
  ...
  // --- Send ---
  try {
    console.log('[ChurnGuard][email] Attempting to send to:', toEmail);
    ...
    if (result?.error) {
      console.error('[ChurnGuard][email] Resend returned error:', result.error);
      return { success: false, error: result.error.message || 'Resend error' };
    }

    console.log('[ChurnGuard][email] ✓ Sent | ID:', result?.data?.id);
    return { success: true, id: result?.data?.id };
  } catch (err) {
    console.error('[ChurnGuard][email] Exception:', err.message);
