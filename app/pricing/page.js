...
const PLANS = [
  {
    ...
    features: [
      ...
      'Full dashboard with all reasons',
      'Email support',
    ],
    highlight: false,
  },
...
export default function PricingPage() {
  ...
  const handleSubscribe = async (planId) => {
    ...
    try {
      ...
      if (window.Paddle && typeof window.Paddle.Checkout?.open === 'function') {
        window.Paddle.Checkout.open({
          transactionId: data.transaction_id,
        });
      } else {
        throw new Error('Paddle.js not loaded yet. Please refresh and try again.');
      }
    } catch (err) {
